...
type BusinessCurrencyRow = {
  businessId: string;
  currency: string;
  _count: { id: number };
  _sum: { amount: number | null; feeAmount: number | null };
};
...
export default async function AdminOverviewPage() {
  ...
  const [
    ...
    activeBusinesses,
    verifiedBusinesses,
    walletConnectedBusinesses,
    webhookConfiguredBusinesses,
    webhookTotal,
    webhookFailed,
    recentTransactions,
    recentBusinesses,
    topBusinessCurrencyStats,
  ] = await Promise.all([
  ...
  const topBusinessMap = topBusinessCurrencyStats.reduce(
    (acc, row: BusinessCurrencyRow) => {
      const existing = acc.get(row.businessId) ?? {
        businessId: row.businessId,
        rows: [] as BusinessCurrencyRow[],
        totalVolume: 0,
        totalFees: 0,
        totalCount: 0,
      };
      ...
      existing.totalVolume += row._sum.amount ?? 0;
      existing.totalFees += row._sum.feeAmount ?? 0;
      existing.totalCount += row._count.id;
      acc.set(row.businessId, existing);
      return acc;
    },
    new Map<
      string,
  ...
  const topBusinessStats = Array.from(topBusinessMap.values())
    .sort((a, b) => b.totalVolume - a.totalVolume)
    .slice(0, 5);

  const topBusinessProfiles = await prisma.businessEntity.findMany({
    where: { id: { in: topBusinessStats.map((item) => item.businessId) } },
    select: { id: true, name: true, contactEmail: true },
  });

  const businessProfileById = new Map(topBusinessProfiles.map((business) => [business.id, business]));
  ...
  const activeBusinessRate = totalBusinesses > 0 ? (activeBusinesses / totalBusinesses) * 100 : 0;
  const deliveryReliability = webhookTotal > 0 ? ((webhookTotal - webhookFailed) / webhookTotal) * 100 : 100;
      ...
      <AdminSurface className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Monitor global volume, platform revenue, business readiness, and transaction health across Trezalink.
          </p>
        </div>
        ...
        {[
          {
            ...
            value: formatCurrencyBreakdown(paidStatsByCurrency, "feeAmount"),
            detail: `24h: ${formatCurrencyBreakdown(dailyStatsByCurrency, "feeAmount")}`,
            tone: "emerald",
          },
          {
            icon: Activity,
            label: "Paid volume",
            value: formatCurrencyBreakdown(paidStatsByCurrency, "amount"),
            detail: `24h: ${formatCurrencyBreakdown(dailyStatsByCurrency, "amount")}`,
            tone: "blue",
          },
          {
            icon: Users,
            ...
            detail: `${activeBusinessRate.toFixed(0)}% active businesses`,
            tone: "red",
          },
          {
            icon: ReceiptText,
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-white/10">
                  {recentTransactions.map((transaction) => (
                    <tr key={transaction.id} className="transition-colors hover:bg-slate-50/80 dark:hover:bg-white/[0.03]">
                      ...
                      <td className="px-5 py-4 font-mono text-xs font-semibold text-slate-900 dark:text-slate-200">{formatCurrencyDisplay(transaction.currency, transaction.amount)}</td>
        ...
        <div className="space-y-6">
          <div className="dashboard-card p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">Business operations readiness</h2>
              </div>
              ...
              {[
                {
                  ...
                  value: `${verifiedBusinesses}/${totalBusinesses}`,
                  detail: "business profile and access confirmed",
                  ok: verifiedBusinesses === totalBusinesses && totalBusinesses > 0,
                },
                {
                  label: "Payment ready",
                  value: `${walletConnectedBusinesses}/${totalBusinesses}`,
                  detail: "settlement wallet linked",
                  ok: walletConnectedBusinesses === totalBusinesses && totalBusinesses > 0,
                },
                {
                  label: "Integration ready",
                  value: `${webhookConfiguredBusinesses}/${totalBusinesses}`,
                  detail: "webhook endpoint configured",
                  ok: webhookConfiguredBusinesses === totalBusinesses && totalBusinesses > 0,
                },
                {
                  label: "Delivery reliability",
                  value: `${deliveryReliability.toFixed(1)}%`,
                  detail: `${webhookFailed}/${webhookTotal} delivery failures`,
                  ok: deliveryReliability >= 95,
                },
              ].map((item) => (
          ...
          <div className="dashboard-card p-5">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {[
                { label: "Volume", value: formatCurrencyBreakdown(weeklyStatsByCurrency, "amount"), icon: TrendingUp },
                {
                  label: "Paid count",
                  value: weeklyStatsByCurrency.reduce((sum, row) => sum + row._count.id, 0).toString(),
                  icon: ReceiptText,
                },
                { label: "Fee yield", value: formatCurrencyBreakdown(weeklyStatsByCurrency, "feeAmount"), icon: Landmark },
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="dashboard-card">
          <div className="flex items-center justify-between border-b border-slate-200 p-5 dark:border-white/10">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Business growth</p>
              <h2 className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">Newest businesses</h2>
            </div>
            <Link href="/admin/businesses" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/[0.06]">
              View all
        ...
        <div className="dashboard-card">
          <div className="flex items-center justify-between border-b border-slate-200 p-5 dark:border-white/10">
            <div>
              ...
              <h2 className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">Top businesses by volume</h2>
            </div>
            <Wallet className="h-5 w-5 text-slate-400" />
          </div>
          <div className="divide-y divide-slate-200 dark:divide-white/10">
            {topBusinessStats.length === 0 ? (
              <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">No paid business volume yet.</div>
            ) : (
              topBusinessStats.map((businessStat, index) => {
                const business = businessProfileById.get(businessStat.businessId);

                return (
                  <div key={businessStat.businessId} className="grid grid-cols-[32px_1fr_auto] items-center gap-4 p-5">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate font-mono text-[10px] text-slate-500 dark:text-slate-400">{business?.contactEmail || businessStat.businessId}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-mono text-sm font-semibold text-slate-950 dark:text-white">{formatCurrencyBreakdown(businessStat.rows, "amount")}</p>
                      <p className="mt-0.5 text-xs text-emerald-600 dark:text-emerald-400">Fee: {formatCurrencyBreakdown(businessStat.rows, "feeAmount")}</p>
                    </div>
                  </div>
                );
              })
