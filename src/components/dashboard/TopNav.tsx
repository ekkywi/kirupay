...
export function TopNav({ merchant }: TopNavProps) {
  ...
  const iconControlProfileClass =
    `${iconControlBaseClass} border-blue-100 bg-blue-50 text-blue-700 hover:bg-blue-100 dark:border-cyan-500/20 dark:bg-blue-500/10 dark:text-cyan-300 dark:hover:bg-blue-500/20`;
  ...
  return (
    <header className="relative z-30 flex h-16 items-center justify-between border-b border-blue-900/10 bg-white/82 px-4 backdrop-blur-xl sm:px-6 lg:px-8 dark:border-white/10 dark:bg-[#080b1f]/90">
      <div className="flex items-center gap-4 min-w-0">
        <div className="lg:hidden inline-flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 via-violet-600 to-cyan-500 text-sm font-bold text-white">
          T
        </div>
      ...
      <div className="flex items-center gap-3">
        {!isAdmin && (
          <div className="relative hidden md:flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-blue-900/10 bg-[#f8faff]/92 px-3 py-2 text-left shadow-sm shadow-blue-950/5 backdrop-blur-2xl transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-70 dark:border-white/10 dark:bg-[#080b1f]/92 dark:shadow-black/20 dark:hover:bg-[#0b1028]/95"
            >
            ...
            <Link
              ...
              className={`rounded-xl border border-blue-900/10 bg-[#f8faff]/92 px-3 py-2 text-[11px] font-semibold shadow-sm shadow-blue-950/5 backdrop-blur-2xl transition-colors dark:border-white/10 dark:bg-[#080b1f]/92 dark:shadow-black/20 ${
                manageDisabled
                  ? "cursor-not-allowed text-slate-400 opacity-70 dark:text-slate-500"
                  : "text-slate-700 hover:bg-white dark:text-slate-200 dark:hover:bg-[#0b1028]/95"
              }`}
            >
              Manage
            </Link>
            ...
            {isBusinessMenuOpen && (
              <div
                ref={businessMenuRef}
                className="absolute left-0 top-[calc(100%+8px)] z-[80] w-[300px] rounded-2xl border border-blue-900/10 bg-white/96 p-2 shadow-2xl shadow-blue-950/15 ring-1 ring-blue-500/10 backdrop-blur-2xl dark:border-white/10 dark:bg-[#080b1f]/98 dark:shadow-black/40 dark:ring-cyan-400/10"
              >
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left transition-colors ${
                          isActive
                            ? "bg-blue-50 text-blue-800 dark:bg-blue-500/10 dark:text-cyan-200"
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className={`truncate text-[11px] ${isActive ? "text-blue-700 dark:text-cyan-300" : "text-slate-500 dark:text-slate-400"}`}>
        ...
        <form onSubmit={handleTransactionSearch} className="relative hidden md:block">
          ...
          <input
            ...
            className="w-64 dashboard-muted-panel py-2 pl-9 pr-3 text-sm text-slate-900 outline-none transition-colors focus:border-blue-500 focus:bg-white dark:border-white/10 dark:bg-white/[0.03] dark:text-white dark:focus:bg-white/[0.05]"
          />
        </form>
        ...
        <div className="relative">
          ...
          {isProfileOpen && (
            <div className="absolute right-0 z-[80] mt-2 w-64 rounded-2xl border border-blue-900/10 bg-white/98 py-2 shadow-2xl shadow-blue-950/15 ring-1 ring-blue-500/10 backdrop-blur-2xl animate-in fade-in zoom-in duration-150 dark:border-white/10 dark:bg-[#080b1f]/98 dark:shadow-black/40 dark:ring-cyan-400/10">
              ...
              <Link
                ...
                className="flex w-full items-center gap-2 px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/[0.06] dark:hover:text-white"
              >
                <Settings size={14} /> {isAdmin ? "Admin Home" : "Settings"}
              </Link>
              <div className="mx-4 my-1 flex items-center gap-2 rounded-lg bg-blue-50/70 px-3 py-2 text-xs text-slate-500 ring-1 ring-blue-900/5 dark:bg-white/[0.06] dark:text-slate-300 dark:ring-white/10">
                <Activity size={14} className="text-emerald-500" />
                {isAdmin ? "Live admin environment" : "Live merchant environment"}
              </div>
              <button 
                onClick={handleLogout}
                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-red-600 transition-colors hover:bg-red-50 hover:text-red-700 dark:text-red-300 dark:hover:bg-red-500/15 dark:hover:text-red-200"
              >
        ...
        createPortal(
          <div className="fixed inset-0 z-[90]">
            ...
            <div
              ...
              className="absolute dashboard-card py-2 shadow-xl shadow-blue-950/10 dark:border-white/10 dark:bg-[#080b1f]/95 dark:shadow-black/30"
              style={{
                        ...
                        {!item.readAt && (
                          <div className="flex shrink-0 flex-col items-end gap-1">
                            ...
                            <button
                              ...
                              className="rounded-md px-2 py-1 text-[11px] font-semibold text-emerald-600 hover:bg-blue-50 dark:text-emerald-300 dark:hover:bg-blue-500/10"
                            >
                              Mark read
                            </button>
                          </div>
