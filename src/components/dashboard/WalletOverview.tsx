    ...
    <div className="relative bg-white dark:bg-[#0B0F17] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-sm shadow-slate-200/60 dark:shadow-none">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-950 dark:text-white">Primary Settlement Wallet</h3>
        </div>
      ...
      {isPending ? (
        <button onClick={handleConnect} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
          {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <> <LinkIcon className="w-4 h-4" /> Connect Wallet</>}
        </button>
      ) : (
        <button onClick={() => setShowConfirmModal(true)} disabled={isLoading} className="w-full bg-red-50 dark:bg-red-500/10 text-red-600 hover:bg-red-100 dark:hover:bg-red-500/20 text-sm font-semibold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
