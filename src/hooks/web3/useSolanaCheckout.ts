...
import { resolveAssetConfig, UsdcConfigError } from "@/lib/asset-config";
...
const TREASURY_SELF_PAYMENT_ERROR = "Payment blocked: buyer wallet cannot be the same as treasury wallet.";
...
function extractWalletErrorMessage(error: WalletError) {
  const nested = (error as WalletError & { cause?: unknown }).cause;
  ...
  return error.message || "Wallet transaction failed.";
}
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (publicKey.toBase58() === treasuryPubKey.toBase58()) {
        throw new Error(TREASURY_SELF_PAYMENT_ERROR);
      }
      ...
      if (transaction.currency === "SOL") {
        ...
        if (payerUsdcAta.toBase58() === treasuryAta.toBase58()) {
          throw new Error(TREASURY_SELF_PAYMENT_ERROR);
        }

        const payerTokenAccountInfo = await connection.getAccountInfo(payerUsdcAta, "confirmed");

        if (!payerTokenAccountInfo) {
          throw new Error("USDC token account not found for payer wallet on current network.");
        }

        const treasuryAccountInfo = await connection.getAccountInfo(treasuryAta, "confirmed");

        if (!treasuryAccountInfo) {
          throw new Error("Treasury USDC token account is not found for current network.");
        }

        const merchantAccountInfo = await connection.getAccountInfo(merchantUsdcAta, "confirmed");
      ...
      if (simulation.value.err) {
        if (simulation.value.err === "AccountNotFound") {
          throw new Error(
            "Transaction simulation failed: required account was not found on active RPC. " +
            "Check payer USDC token account, treasury USDC ATA, and wallet RPC/cluster alignment."
          );
        }
        const logs = simulation.value.logs?.slice(-6).join(" | ") || "No simulation logs.";
      ...
      if (err instanceof WalletError) {
        displayMessage = extractWalletErrorMessage(err);
      } else if (err instanceof UsdcConfigError) {
        const missing = err.missingKeys?.length ? ` Missing: ${err.missingKeys.join(", ")}.` : "";
        displayMessage = `USDC checkout is not configured for ${err.cluster}.${missing}`;
      } else if (err instanceof Error) {
        displayMessage = err.message;
      }

      if (process.env.NODE_ENV === "development") {
        if (err instanceof UsdcConfigError) {
          console.warn("[Solana Checkout Config Warning]:", {
            code: err.code,
            cluster: err.cluster,
